...
download() {
    local url="$1" out="$2"
    rm -f "$out"
    if have wget; then wget -O "$out" "$url" || return 1; return 0; fi
    if have curl; then curl -L --fail -o "$out" "$url" || return 1; return 0; fi
    return 1
}
...
[ -f files/usr/bin/podkop-telegram-agent ] || fail "bad archive: files/usr/bin/podkop-telegram-agent missing. Re-upload FULL package, not patch-only archive."
[ -f files/etc/init.d/podkop-telegram-agent ] || fail "bad archive: files/etc/init.d/podkop-telegram-agent missing. Re-upload FULL package, not patch-only archive."
